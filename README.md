...
- **Table of contents** (`T` to toggle, or `:toc`) — popup tree of mupdf's outline. Defaults to collapsed-with-current-page-ancestry-expanded, so you see chapters with the section you're in opened. `l`/`h` (or `→`/`←`) expand/collapse; `L`/`H` expand/collapse all; Space toggles; Enter jumps; mouse wheel, `j`/`k`, and animated `Ctrl+D`/`Ctrl+U` half-page jumps navigate; `g`/`G` for top/bottom.
